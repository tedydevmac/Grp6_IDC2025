        ...
        for i in range(count):
            ...
            if box:
                ...
                for old_box, old_item in detected_medical_objects:
                    if old_item == item and calculate_iou(box, old_box) > 0.15:  # Higher IoU threshold for same object
                        is_new_object = False
                        break
...
def detect_items(frame, item_classes):
    ...
    """
    results = model.predict(frame, conf=0.65, imgsz=512)  # Adjust confidence threshold as needed
    ...
    for result in results:
        for box in result.boxes:
            ...
            if cls in item_classes:
                ...
                is_duplicate = any(calculate_iou(box_coords, existing_box) > 0.15 for existing_box, _ in detected_boxes)
